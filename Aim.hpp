...
struct Aim {
    ...
    void StartAiming() {
        ...
        // Introduce randomization for more human-like aim
        std::random_device rd;
        std::mt19937 gen(rd());
        std::uniform_real_distribution<> dis(-0.5, 0.5);

        double totalPitchIncrement = aimPitchIncrement + nrPitchIncrement + dis(gen);
        double totalYawIncrement = aimYawIncrement + nrYawIncrement + dis(gen);
